...
class DaViT(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000, depths=(1, 1, 3, 1), patch_size=4,
        ...
        # flops : 2312185856
        # param : 15401064
        num_heads = (3, 6, 12, 24) # 또는 (2, 4, 8, 16)
        ...
        # make half only using double branches : dpr을 1/2 size로 만들어야 됨.
        ...
        self.patch_embeds = nn.ModuleList([
            PatchEmbed(patch_size=patch_size if i == 0 else 2,
                       in_chans=in_chans if i == 0 else self.embed_dims[i - 1],  # chans : //2 버전이 아닌, 그냥 버전 사용
        ...
        # after all blocks
        self.norms = norm_layer(self.embed_dims[-1]) # chans : //2 버전이 아닌, 그냥 버전 사용
        self.avgpool = nn.AdaptiveAvgPool1d(1)
        self.head = nn.Linear(self.embed_dims[-1], num_classes)  # chans : //2 버전이 아닌, 그냥 버전 사용
        ...
        for block_id, block_param in enumerate(self.architecture):
            ...
            fusion_block = nn.ModuleList([
                MySequential(*[
                    Fusion(dim=self.embed_dims[item],
                           mlp_ratio=mlp_ratio,
                           drop_path=dpr[layer_id + layer_offset_id],
                           norm_layer=nn.LayerNorm,
                           ffn=ffn)]
        ...
        for block_index, block_param in enumerate(self.architecture):
            ...
            for layer_index, branch_id in enumerate(block_param):
                ...
                if switch_flag:
                    ...
                    x1, _ = self.main_blocks[block_index][layer_index][0](features[branch_id], sizes[branch_id])
                    x2, _ = self.main_blocks[block_index][layer_index][1](features[branch_id], sizes[branch_id])
                    x = x1 + x2
                    ...
                    x2, _ = self.main_blocks[block_index][layer_index][0](features[branch_id], sizes[branch_id])
                    x1, _ = self.main_blocks[block_index][layer_index][1](features[branch_id], sizes[branch_id])
                    x = x1 + x2
