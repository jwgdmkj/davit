...
'''
feature -- 1/2로 나눠, spatial_attn -- (1x1) conv로 channel recover -- (3x3) -- elementwise sum
        |- 1/2로 나눠, channel_attn -- (1x1) conv로 channel recover  |

embed_dims = (128, 256, 512, 1024)
num_heads = (4, 8, 16, 32)
--> flops : 2312185856, param : 15401064
'''
...
class DaViT(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000, depths=(1, 1, 3, 1), patch_size=4,
        ...
        self.conv1 = nn.ModuleList([
            nn.Conv2d(self.embed_dims[i], embed_dims[i], kernel_size=1, stride=1, padding=0)
            for i in range(self.num_stages)
        ])
        ...
        self.conv33 = nn.ModuleList([
            nn.Conv2d(embed_dims[i], embed_dims[i], kernel_size=3, stride=1, padding=1)
            for i in range(self.num_stages)
        ])
    ...
    def forward(self, x):
        ...
        for block_index, block_param in enumerate(self.architecture):
            ...
            for layer_index, branch_id in enumerate(block_param):
                ...
                x1 = x1.reshape(x1.size(0), x1.size(2), sizes[branch_id][0], sizes[branch_id][1]).contiguous()
                x2 = x2.reshape(x2.size(0), x2.size(2), sizes[branch_id][0], sizes[branch_id][1]).contiguous()

                x1 = self.conv1[block_index](x1)
                x2 = self.conv2[block_index](x2)

                # features[branch_id] = x1 + x2

                x = x1 + x2
                features[branch_id] = self.conv33[block_index](x).flatten(2).transpose(1, 2)
        # ---------------------------------self attn stage ---------------------------------------------- # 
