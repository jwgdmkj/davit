...
class Fusion(nn.Module):      
    ...
    def __init__(self, dim):
        ...
        self.conv1 = nn.Conv2d(dim // 2, dim, kernel_size=1, stride=1, padding=0)
        self.conv2 = nn.Conv2d(dim // 2, dim, kernel_size=1, stride=1, padding=0)

    def forward(self, x1, x2, switch_flag, size):
        x1 = x1.transpose(1, 2).view(x1.size(0), x1.size(1), size[0], size[1])
        x2 = x2.transpose(1, 2).view(x2.size(0), x2.size(1), size[0], size[1])
