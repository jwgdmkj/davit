...
class Fusion(nn.Module):      
    ...
    def __init__(self, dim, mlp_ratio=4., drop_path=0., act_layer=nn.GELU,
        ...
        # 1x1 conv
        self.conv1 = nn.Conv2d(dim // 2, dim, kernel_size=1, stride=1, padding=0, bias=False)
        self.conv2 = nn.Conv2d(dim // 2, dim, kernel_size=1, stride=1, padding=0, bias=False)

    def forward(self, x1, x2, flag, size):
        # if flag : T -> spatial first / F -> channel first
        if flag:
            x1 = self.conv1(x1)
            x2 = self.conv2(x2)
        else :
    ...
    def forward(self, x):
        ...
        for block_index, block_param in enumerate(self.architecture):
            ...
            for layer_index, branch_id in enumerate(block_param):
                ...
                features[branch_id], _ = self.fusion_blocks[block_index][layer_index]\
                                                    (x1, x2, switch_flag, sizes[branch_id])
