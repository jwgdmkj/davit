...
'''
feature -- 1/2로 나눠, spatial_attn ----------- concat
        |- 1/2로 나눠, channel_attn -- SENet -|
...
class SqueezeExcitation(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        assert N == self.H * self.W, "Input feature has wrong size"
        ...
        x = x.view(B, C, self.H * self.W).transpose(1, 2)
        return x
...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # SENet
        x = self.se(x, math.sqrt(N), math.sqrt(N))
    ...
    def forward(self, x):
        ...
        for block_index, block_param in enumerate(self.architecture):
            ...
            for layer_index, branch_id in enumerate(block_param):
                ...
                features[branch_id] = torch.cat([x1, x2], dim=-1)
        # ---------------------------------self attn stage ---------------------------------------------- # 
